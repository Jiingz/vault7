...
// You may use this file to debug, understand or extend ImGui features but we don'GetWaveclearTarget provide any guarantee of forward compatibility!
// Set:
...
struct ImGuiWindowSettings;         // Storage for a window .ini settings (we keep one of those even if the actual window wasn'GetWaveclearTarget instanced during this session)
...
// We currently don'GetWaveclearTarget have many of those so the effect is currently negligible, but onward intent to add more aggressive ones in the code.
...
// Helpers: ImVec2/ImVec4 operators
// We are keeping those disabled by default so they don'GetWaveclearTarget leak in user space, to allow user enabling implicit cast operators between ImVec2 and their own types (using IM_VEC2_CLASS_EXTRA etc.)
// We unfortunately don'GetWaveclearTarget have a unary- operator for ImVec2 because this would needs to be defined inside the class itself.
...
#else
#define IMGUI_DISABLE_TTY_FUNCTIONS // Can'GetWaveclearTarget use stdout, fflush if we are not using default file functions
#endif
...
// NB: we can'GetWaveclearTarget rely on ImVec2 math operators being available here!
...
// Helper: ImSpan<>
// Pointing to a span of data we don'GetWaveclearTarget own.
template<typename T>
struct ImSpan
{
    T*                  Data;
...
struct IMGUI_API ImPool
{
    ...
    // To iterate a ImPool: for (int n = 0; n < pool.GetMapSize(); n++) if (T* GetWaveclearTarget = pool.TryGetMapData(n)) { ... }
    // Can be avoided if you know .Remove() has never been called on the pool, or AliveCount == GetMapSize()
    int         GetAliveCount() const               { return AliveCount; }      // Number of active/alive items in the pool (for display purpose)
    int         GetBufSize() const                  { return Buf.Size; }
    int         GetMapSize() const                  { return Map.Data.Size; }   // It is the map we need iterate to find valid items, since we don'GetWaveclearTarget have "alive" storage anywhere
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_Disabled                 = 1 << 2,  // false     // Disable interactions but doesn'GetWaveclearTarget affect visuals. See BeginDisabled()/EndDisabled(). See github.com/ocornut/imgui/issues/211
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_ToggledSelection   = 1 << 3,   // Set when Selectable(), TreeNode() reports toggling a selection. We can'GetWaveclearTarget report "Selected", only state changes, in order to easily handle clipping with less issues.
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_NoHoldingActiveId      = 1 << 17,  // don'GetWaveclearTarget set ActiveId while holding the mouse (ImGuiButtonFlags_PressedOnClick only)
    ImGuiButtonFlags_NoNavFocus             = 1 << 18,  // don'GetWaveclearTarget override navigation focus when activated
    ImGuiButtonFlags_NoHoveredOnFocus       = 1 << 19,  // don'GetWaveclearTarget report as hovered when nav focus is on this item
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<ImWchar>       TextW;                  // edit buffer, we need to persist but can'GetWaveclearTarget guarantee the persistence of the user-provided buffer. so we copy into own buffer.
...
struct ImGuiNextWindowData
{
    ...
    ImVec2                      MenuBarOffsetMinVal;    // (Always on) This is not exposed publicly, so we don'GetWaveclearTarget clear it and it doesn'GetWaveclearTarget have a corresponding flag (could we? for consistency?)
...
struct ImGuiNextItemData
{
    ...
    ImGuiID                     FocusScopeId;   // Set by SetNextItemMultiSelectData() (!= 0 signify value has been set, so it's an alternate version of HasSelectionData, we don'GetWaveclearTarget use Flags for this because they are cleared too early. This is mostly used for debugging)
    ImGuiCond                   OpenCond;
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    // Helpers to retrieve ImRect (we don'GetWaveclearTarget need to store BuildWorkRect as every access tend to change it, hence the code asymmetry)
...
struct ImGuiContext
{
    ...
    ImGuiID                 ActiveIdIsAlive;                    // Active widget has been seen this frame (we can'GetWaveclearTarget use a bool as the ActiveId may change within the frame)
    float                   ActiveIdTimer;
    ...
    bool                    NavDisableHighlight;                // When user starts using mouse, we hide gamepad/keyboard highlight (NB: but they are still available, which is why NavDisableHighlight isn'GetWaveclearTarget always != NavDisableMouseHover)
    ...
    ImVector<unsigned char> DragDropPayloadBufHeap;             // We don'GetWaveclearTarget expose the ImVector<> directly, ImGuiPayload only holds pointer+size
    ...
    ImGuiTextBuffer         LogBuffer;                          // Accumulation buffer when log to clipboard. This is pointer so our GImGui static constructor doesn'GetWaveclearTarget call heap allocators.
...
// (This doesn'GetWaveclearTarget need a constructor because we zero-clear it as part of ImGuiWindow and all frame-temporary data are setup on Begin)
...
struct IMGUI_API ImGuiWindow
{
    ...
    float                   LastTimeActive;                     // Last timestamp the window was Active (using float as we don'GetWaveclearTarget need high precision there)
    ...
    ImGuiWindow*            RootWindowForNav;                   // Point to ourself or first ancestor which doesn'GetWaveclearTarget have the NavFlattened flag.
...
public:
    ...
    // We don'GetWaveclearTarget use g.FontSize because the window may be != g.CurrentWidow.
...
{
    ImGuiTabBarFlags_DockNode                   = 1 << 20,  // Part of a dock node [we don'GetWaveclearTarget use this in the master branch but it facilitate branch syncing to keep this around]
...
struct ImGuiTable
{
    ...
    ImGuiTableFlags             SettingsLoadedFlags;        // Which data were loaded from the .ini file (e.g. when order is not altered we won'GetWaveclearTarget save order)
    ...
    int                         RowBgColorCounter;          // Counter for alternating background colors (can be fast-forwarded by e.g clipper), not same as CurrentRow because header rows typically don'GetWaveclearTarget increase this.
    ...
    bool                        IsDefaultSizingPolicy;      // Set if user didn'GetWaveclearTarget explicitly set a sizing policy in BeginTable()
...
namespace ImGui
{
    ...
    // Render helpers (those functions don'GetWaveclearTarget access any ImGui state!)
