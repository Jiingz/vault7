...
bool MainMenu::open_ = false;
int MainMenu::delay_ = 0;
MainMenu::active_menu MainMenu::active_menu_ = MainMenu::active_menu::coreMenu;
std::once_flag MainMenu::style_initialized_;

void MainMenu::CustomColor()
{
...
{
	std::call_once(style_initialized_, []()
		{
			CustomStyle();
			CustomColor();
		});
	...
	if (GetAsyncKeyState(VK_INSERT) && GetTickCount() > delay_)
	{
		open_ = open_ == true ? false : true;
		delay_ = GetTickCount() + 200;
	}

	if (!open_)
		return;

	ImGui::Begin(xor ("Vault7"), &open_, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);

	ImGui::SetCursorPos({ -27.f,11.f });

	ImGui::BeginChild(xor ("child0"), { 152.f,470.f }, true);

	ImGui::SetCursorPos({ 29.f,26.f });
	if (ImGui::Button(xor ("CORE"), { 152.f,19.f }))
	{
		active_menu_ = active_menu::coreMenu;
	}
	ImGui::SetCursorPos({ 29.f,76.f });
	if (ImGui::Button(xor ("ORBWALKER"), { 152.f,19.f }))
	{
		active_menu_ = active_menu::orbwalker;
	}
	ImGui::SetCursorPos({ 25.f,126.f });
	if (ImGui::Button(xor ("CHAMPION"), { 152.f,19.f }))
	{

	}
	ImGui::SetCursorPos({ 29.f,179.f });
	if (ImGui::Button(xor ("NAME CHANGER"), { 152.f,19.f }))
	{
